...
public class PokerHand {
    ...
    public void evalHand(){
        ...
        hand.forEach(card -> faceList.add(card.faceField()));

        List<String> duplicatedFaceCards = new ArrayList<>();
        faceList.forEach(face -> {
            int count = Collections.frequency(faceList, face);
            if (!duplicatedFaceCards.contains(face) && count > 1) duplicatedFaceCards.add(face);
        });

        for (String duplicate:duplicatedFaceCards) {
    ...
    public String toString() {
        return "%d. %-16s Rank:%d %-40s %s".formatted(playerNo, score, score.ordinal(), hand, (!discards.isEmpty()) ? "Discards:"+ discards: "");
    }
}
