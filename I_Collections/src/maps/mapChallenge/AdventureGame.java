...
public class AdventureGame {
    ...
    private record Location (String description, Map<Compass, String> nextpPlaces){}
    ...
    private Map<Compass, String> loadDirections(String nextPlaces){
       Map<Compass, String> directions = new HashMap<>();
        ...
        for (String nextPlace : nextSteps){
            ...
            String dest = splits[1].trim();
            directions.put(compass, dest);
        }
        return directions;
    }
    private void visit(Location location){
        System.out.printf("*** You're Standing at %s *** %n", location.description);
        System.out.println("\t From here, you can see: ");

        location.nextpPlaces.forEach((k,v) -> {
            System.out.printf("\t • A  %s to the %s (%S) %n", v,k.getString(), k);
        });
        System.out.println("Select your compass (Q to quit) >>");
    }
    public void move(String direction){
        var nextPlaces = adventureMap.get(lastPlace).nextpPlaces;
        String nexplace = null;
        if("ENSW".contains(direction)){
            nexplace = nextPlaces.get(Compass.valueOf(direction));
            if (nexplace != null){
                play(nexplace);
            }
        }else {
